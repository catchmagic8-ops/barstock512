...
const emptyForm: ItemForm = { name: "", category: "spirits", unit: "bottles", quantity: "0", minStock: "2" };
...
export default function StockManager() {
  ...
  const handleAdd = () => {
    ...
    addItem.mutate(
      { id, name: form.name.trim(), category: form.category, unit: form.unit.trim() || "units", quantity: parseInt(form.quantity) || 0, min_stock: parseInt(form.minStock) || 0 },
      {
  ...
  const startEdit = (item: InventoryItem) => {
    ...
    setEditForm({ name: item.name, category: item.category, unit: item.unit, quantity: String(item.quantity), minStock: String(item.minStock) });
  };

  const handleEdit = () => {
    if (!editingId || !editForm.name.trim()) return;
    editItem.mutate(
      { id: editingId, name: editForm.name.trim(), category: editForm.category, unit: editForm.unit.trim(), quantity: parseInt(editForm.quantity) || 0, min_stock: parseInt(editForm.minStock) || 0 },
      {
